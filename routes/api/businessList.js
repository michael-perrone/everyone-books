...
router.post("/businessSearch", async (req, res) => {
  if (
    ...
    let businessesByName = await Business.find({ typeOfBusiness: req.body.typeOfBusiness }).select(['schedule', '_id', 'businessName', 'address', 'state', 'city', 'zip', 'website', 'phoneNumber', "eq"]);
    if (businessesByName) {
