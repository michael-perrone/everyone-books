...
const WeekSelector = (props) => {
   ...
   function addYear() {
    ...
    return (
        ...
        <div id={styles.days}>
           <div><p style={{marginBottom: '8px'}}>Sun</p>{sunday.map(element => <p style={{ height: element === " " ? "18px": "", backgroundColor: dayNum === element.day && (element.type !== "premonth" && element.type !== "postmonth") ? "black": "", color: dayNum === element.day && (element.type !== "premonth" && element.type !== "postmonth") ? "white" : "" }} onClick={getDate(element.day, element.type)} id={element.type === "premonth" || element.type === "postmonth" ? styles.preMonthPostMonth : styles.regDay} className={styles.day}>{element.day}</p>)}</div>
